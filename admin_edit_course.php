...
$page_subtitle = 'تحديث بيانات الدورة التعليمية';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = mysqli_prepare($conn, "UPDATE courses SET title=?, price=?, description=?, track_id=?, image=?, status=? WHERE id=?");
        mysqli_stmt_bind_param($stmt, "sssissi", $title, $price, $description, $track_id, $image_name, $status, $course_id);

        if (mysqli_stmt_execute($stmt)) {
            header("Location: admin_courses.php?msg=updated");
            exit();
        } else {
...
<div class="admin-form-card">
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <div class="form-row">
            ...
            <input type="text" name="price" id="price" value="<?php echo htmlspecialchars($course['price']); ?>" required>
        </div>
        <div class="form-row">
            ...
            <select name="track_id" id="track_id" required>
                ...
                <?php while ($t = mysqli_fetch_assoc($tracks)): ?>
                    <option value="<?php echo $t['id']; ?>" <?php echo ($course['track_id'] == $t['id']) ? 'selected' : ''; ?>>
        ...
        <div class="form-row">
            ...
            <textarea name="description" id="description" rows="4" required><?php echo htmlspecialchars($course['description']); ?></textarea>
        </div>
        <div class="form-row">
            <label for="status"><i class="fas fa-toggle-on"></i> الحالة</label>
            <select name="status" id="status">
                <option value="active" <?php echo ($course['status'] ?? 'active') === 'active' ? 'selected' : ''; ?>>نشطة</option>
                <option value="inactive" <?php echo ($course['status'] ?? 'active') === 'inactive' ? 'selected' : ''; ?>>غير نشطة</option>
            </select>
        </div>
        <div class="form-row">
            ...
            <?php if (!empty($course['image']) && $course['image'] !== 'default.jpg'): ?>
                <div class="current-image-preview">
                    <p style="font-size:0.8rem; color:var(--admin-text-muted); margin-top:6px;">الصورة الحالية:</p>
                    <img src="uploads/<?php echo htmlspecialchars($course['image']); ?>" alt="صورة الدورة">
                </div>
            <?php endif; ?>
        </div>
        <div class="form-actions">
            <button type="submit" class="btn-admin btn-save"><i class="fas fa-floppy-disk"></i> حفظ التعديلات</button>
